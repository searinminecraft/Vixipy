...
async def ugoira_proxy(id: int):
    endpoint = current_app.config.get("UGOIRA_ENDPOINT") or "https://t-hk.ugoira.com/ugoira/%s.mp4"
    ...
    r: ClientResponse = await current_app.content_proxy.get(endpoint % id, headers={
        "Referer": referer
    })
    r.raise_for_status()
