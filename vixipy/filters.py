...
def filter_from_prefs(works: list[Union[ArtworkEntry, NovelEntry, NovelSeriesEntry]]):
    ...
    hide_sensitive = current_app.config["NO_SENSITIVE"] or bool(int(request.cookies.get("Vixipy-No-Sensitive", 0)))
    ...
    for x in works:
        ...
        if isinstance(x, ArtworkEntry) and x.sensitive and hide_sensitive:
            ...
            new_works.remove(x)
            continue
        if (hide_r18_instancewide and x.xrestrict >= 1) or (hide_r18 and x.xrestrict >= 1):
            log.debug("Removing %s because it is R18", x)
            new_works.remove(x)
            continue
        if (hide_r18_instancewide and x.xrestrict >= 1) or ((hide_r18 and x.xrestrict != 0) or (hide_r18g and x.xrestrict == 2)):
